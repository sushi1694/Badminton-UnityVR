...
public class Opponent : MonoBehaviour
{
    ...
    // New field for miss threshold (randomly chosen between 5 and 10)
    ...
    void Start()
    {
        ...
        // Initialize the miss threshold (random integer between 5 and 10 inclusive)
        missThreshold = Random.Range(5, 11);
    }
    ...
    {
        float z = (target.transform.position.z > opponent.transform.position.z)
                    ? opponent.transform.position.z + intervalLength
                    : opponent.transform.position.z - intervalLength;
    ...
    void attackPlayer()
    {
        ...
        {
            hits++;
            // After a set number of hits, the opponent has a chance to miss.
            if (hits >= missThreshold)
            {
                Debug.Log("Opponent misses the shuttle!");
                // Reset miss threshold for next miss cycle.
                missThreshold = Random.Range(5, 11);
                // Update the shuttle's last hitter to Player so scoring awards the player.
                ShuttleHitterTracker tracker = shuttle.GetComponent<ShuttleHitterTracker>();
                if (tracker != null)
                {
                    tracker.lastHitter = Hitter.Player;
                }
                // Let the shuttle fall naturally by enabling gravity.
    ...
    bool playerReceive()
    {
        return PlayerRacket.GetComponent<Player>().getIsLanded();
    }

    void attack()
    {
        // Update last hitter to Opponent (for normal attacks)
        ShuttleHitterTracker tracker = shuttle.GetComponent<ShuttleHitterTracker>();
        if (tracker != null)
        {
            tracker.lastHitter = Hitter.Opponent;
        }
    ...
    (float, float) setTargetPoints()
    {
        ...
        float x = lowerBoundX + ((upperBoundX - lowerBoundX) * (float)rnd.NextDouble());
        float z = lowerBoundZ + ((upperBoundZ - lowerBoundZ) * (float)rnd.NextDouble());
        return (x, z);
    }
